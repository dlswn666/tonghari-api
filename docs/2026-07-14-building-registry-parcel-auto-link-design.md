...
created_at timestamptz NOT NULL DEFAULT clock_timestamp()
PRIMARY KEY (operation_id, input_token_hash, pnu)
FK (operation_id, union_id) -> building_write_operations(id, union_id) ON DELETE RESTRICT
```

전용 append RPC는 `input_token_hash`가 DB root `request_manifest.explicit_input_tokens`에 존재하는지, resolution evidence가 같은 token/PNU를 가리키는지 검증한다. 서로 다른 원본 token이 같은 PNU로 해소되어도 두 token의 provenance를 모두 보존해야 하므로 PK에는 `input_token_hash`를 포함한다. 같은 token/PNU/evidence replay만 기존 행을 재사용하고, 같은 PNU라는 이유만으로 서로 다른 token의 evidence를 합치지 않는다. API relation에서만 발견된 PNU는 이 원장에 넣지 않고 scan pair에만 둔다. canonical RPC는 `explicit_scope_pnus`가 이 원장의 해당 operation PNU subset과 exact match인지 검증하고, command 분할 시 command에 선언되지 않은 나머지 원본 PNU가 있어도 허용하되 **원장에 없는 PNU 하나라도 explicit로 제출하면 전체 요청을 거부**한다. 따라서 caller boolean이나 adapter 관례가 아니라 DB evidence가 standalone BUILDING_REGISTER/GIS_DIRECT 허용 경계다.
...
상태 CHECK는 `PENDING → result_status/completed_at NULL`, `RETRYABLE → result_status='RETRY_REQUIRED' AND completed_at NULL`, `COMPLETED → 위 matrix의 terminal result AND completed_at NOT NULL`, `FAILED → result_status='LOCK_SET_UNSTABLE' AND completed_at NOT NULL`을 강제한다. `RETRY_REQUIRED`에서는 source/building/projection/event를 쓰지 않고 `attempt_count+1`과 최신 required lock payload만 저장한다. payload는 정렬·중복 제거된 `required_lock_pnus`, `required_lock_external_identity_keys`, 8KB 이하 object인 `diagnostics`의 exact 세 key만 허용한다. 재시도 attempt identity와 response-loss replay 비교에는 두 lock 배열만 사용하고 진단 문구는 제외한다. 확장 lock-only 배열 재호출은 같은 command key/hash로 실행할 수 있다. terminal command 결과 저장은 source/projection/event와 같은 transaction에서 수행한다. 권한 회수 또는 job archive 뒤에도 이미 끝난 `COMPLETED/FAILED`의 exact replay만 저장 결과를 읽어 반환할 수 있고, `PENDING/RETRYABLE` replay와 모든 새 상태 전이는 현재 actor 권한과 active job 상태를 다시 검증한다.

`sync_jobs`에는 사전 검증 후 `union_id SET NOT NULL`, `UNIQUE(id, union_id)`, archive 감사 컬럼을 추가한다. GIS queue는 `sync_jobs`와 operation의 영속화가 모두 성공한 뒤에만 admission한다. 둘 중 하나라도 실패하면 memory job을 제거하고 `SYNC_JOB_PERSIST_FAILED`로 fail-closed한다. `resetGisData`는 참조된 job을 DELETE하지 않고 `archived_at`, `archived_by_user_id`, `archive_reason`을 기록한다. PROCESSING job 또는 연결 operation에 `PENDING/RETRYABLE` command가 하나라도 있는 job은 archive하지 않는다. archive 완료 행은 역사 snapshot이므로 이후 status/progress/preview/result를 포함한 전체 UPDATE와 DELETE를 trigger로 거부한다. trigger/RLS를 우회하는 `TRUNCATE`와 schema 변조에 불필요한 `REFERENCES`, `TRIGGER` table privilege는 PUBLIC/anon/authenticated/service_role에서 회수한다. 최초 archive transition만 전용 RPC가 수행한다.
...
8. 새로 발견한 전역 PNU 또는 canonical external identity key가 입력 lock 집합에 없으면 어떤 source/building/unit/projection/event도 쓰지 않고 `RETRY_REQUIRED(required_lock_pnus, required_lock_external_identity_keys, diagnostics)`를 반환한다. `required_lock_pnus`에는 타 조합에만 존재하는 필지도 포함될 수 있다. `diagnostics`는 8KB 이하 object로 원인을 남기되 attempt identity에는 포함하지 않는다. source payload와 command hash는 그대로이고 identity를 구성하는 lock-only 두 배열만 확장한다.
