...
- artifact는 PNU, job ID, 상태, count, issue code, digest만 포함하며 토큰, secret,
  사용자 UUID, 이름, 주소, 연락처, 원문 응답을 포함하지 않는다.
...
입력 UUID는 이 secret과 exact 일치해야 한다. JWT secret과 개발 service-role key는
GitHub runner로 전달하지 않고 현재 EC2 컨테이너 환경에서만 사용한다.
...
durable job이 `COMPLETED` 또는 `FAILED`가 될 때까지 drain한다. terminal 확인 후에만
...
`latest`의 anchor 또는 `sourceDiscoveryJobId` lineage로 durable admission을 복구할 때까지
lock을 보유하고, 복구한 job을 같은 terminal drain에 연결한다.
...
임시 파일도 같은 방식으로 삭제와 디렉터리 부재를 확인한다.
