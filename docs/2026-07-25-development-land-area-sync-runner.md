...
컨테이너를 재기동할 수 없다. runner는 최대 2,400초로 제한되며 SSH/timeout 종료 시
커널이 lock을 해제한다. GitHub concurrency group은 pending run을 대체할 수 있으므로
직렬화 권위로 사용하지 않는다.
...
8. 개발 feature flag와 allowlist를 후속 disable workflow로 원복
9. 운영 DB write 0
