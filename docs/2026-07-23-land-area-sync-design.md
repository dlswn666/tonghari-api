...
- 작성일: 2026-07-23
- 상태: 구현 전 최종 설계 · 2026-07-23 독립 검증 반영
- API 기준선: `tonghari-api` `origin/main` `e1e7ba7678ab6462a9484836d6c092150e24f5ec`
- Web 기준선: `tonghari-web` `origin/master` `81dc82274409b9a21a46a03304436104b53a2b19`
- 기준 노트:
...
9. 모든 주택 유형은 분류 전에 기준·부속지번 완전성 gate를 통과해야 한다. positive-cache 관계 부재, 자체 표제부 존재, `bylotCnt=0`, anchor 기준 attached 0건 중 어느 조합도 “다른 건물의 부속 PNU가 아님”을 증명하지 못한다.
...
- Building HUB 표제부·전유부·부속지번 `getBrAtchJibunInfo` strict adapter와 주택 유형 분기
...
- 조합원 화면의 `적용 토지면적`과 `필지 전체면적` 라벨·원천 분리
...
| 건축HUB 건축물대장정보 | 표제부·전유부·관리 PK·`bylotCnt`·`getBrAtchJibunInfo`의 `atch*` 필드 제공 | 대장 구분을 1차 분류, 용도 코드는 exact allowlist, same-run 필지 범위 완전성 증명 |
...
flowchart TD
    ...
    D --> E["anchor Building HUB title strict 전체 페이지"]
    E --> F["DB 전체 evidence resolver<br/>positive + blocking + OPEN unresolved connected component"]
    F --> G["getBrAtchJibunInfo strict 전체 페이지<br/>LINKED는 모든 distinct base PNU, no-cache는 anchor 조회"]
    ...
    H -->|"LINKED + current scan exact"| I["LINKED scope의 title/expos strict 전체 페이지"]
    Y --> I2["anchor scope의 title/expos strict 전체 페이지"]
...
분류 전에 anchor discovery 모집단을 먼저 완성한다.

- anchor PNU의 Building HUB 표제부 전 페이지
- positive-cache VIEW의 anchor 기준·부속 양방향 `PENDING`·`LINKED` positive evidence
- 원천 relation·manual override의 review/conflict/stale evidence와 관련 OPEN unresolved evidence
- 같은 실행의 `getBrAtchJibunInfo` 전 페이지
- `bylotCnt`, distinct attached PNU 수, 관리번호의 상호 일치
- root 관리번호와 기준·부속 edge로 만든 bounded connected component
...
| `bylotCnt`와 distinct attached PNU 수 불일치 | 없음 | `REVIEW_REQUIRED` |
...
### 10.4 `getBrAtchJibunInfo` 계약

- land-area sync는 auto-link 파이프라인의 API 호출 최적화보다 엄격하게 동작하며 `bylotCnt=0`이어도 attached endpoint를 조회한다.
...
- `LINKED` component에서는 resolver가 식별한 모든 distinct `base_pnu`를 각각 strict 전 페이지 조회하고 결과를 합친다. anchor가 attached PNU여도 해당 base와 sibling 전체를 검사하며 base 하나라도 실패·불완전하면 전체 `FAILED`다.
- no-cache component만 anchor를 직접 조회하며, 그 zero는 아래 관리자 확인 후보 이상으로 승격하지 않는다.
- `bylotCnt=0 + ATTACHED_COMPLETE_ZERO + blocking evidence 없음`은 관리자 확인 가능한 single candidate일 뿐 단일 PNU 증명이 아니다.
- `bylotCnt=0 + attached row 또는 기존 PENDING/LINKED evidence`는 conflict다.
- `bylotCnt>0`, null, 누락, 파싱 실패에는 complete attached rows가 필요하다.
- `bylotCnt`와 같은 관리번호의 distinct attached PNU 수가 다르면 `REVIEW_REQUIRED`다.

### 10.5 기존 inspector 경계
...
API의 `resolveParcelScopeCompleteness()`는 DB resolver 결과와 same-run title/attached strict scan을 결합해 다음 중 하나만 반환한다.
...
- cache와 blocking evidence가 없고 attached가 `ATTACHED_COMPLETE_ZERO`, 명시적 `bylotCnt=0`, root 모순이 없어도 `SINGLE_SCOPE_CONFIRMATION_REQUIRED`다. 자체 title은 다른 건물의 부속이 아님을 증명하지 않는다.
...
- `bylotCnt`, attached distinct count, cache PNU, root identity가 모순되면 `REVIEW_REQUIRED`다.
...
- `externalScopeDigest`: same-run title/attached의 endpoint·completeness·`totalCount`·정렬 canonical identity rows. 요청시각·secret·provider raw body는 제외
...
- `bylotCnt`, attached scan, connected component, root identity의 모순이 없어야 한다.
...
- 후속 job에서 title/attached/resolver와 해당 전략의 모든 scan·매칭을 다시 실행
...
- scope state, `bylotCnt`, component·attached PNU 수, PENDING/LINKED/blocking/OPEN unresolved 수
- 실행 버튼
...
| --- | --- | --- | --- |
| `land_area` | `property_units.land_area` | `적용 토지면적` | 없음 |
...
`MemberEditModal`에서는 같은 `pu.land_area`를 건물 정보의 `대지권 면적`과 토지 정보의 `소유면적`으로 중복 노출하지 않는다. 토지 정보의 `적용 토지면적` 한 곳으로 통합하고 source badge·동기화 시각을 함께 표시한다.

도움말은 source에 따라 바꾼다.

- 자동값: `공공 원천에서 자동 수집된 적용 토지면적입니다. 수동으로 변경하면 출처가 '수동'으로 전환됩니다.`
- 수동·출처미확인: `단독·다가구는 토지대장, 다세대는 대지권등록부 또는 등기자료를 확인해 입력하세요.`

숫자 검증 오류, audit diff label, Excel(XLSX) export header도 `대지권 면적` 고정 문구 대신 `적용 토지면적`을 사용한다.
...
- `적용 토지면적` 정렬·숫자 필터 key는 삭제되지 않은 property별 `land_area`의 합이다.
...
- 실제 수동 변경이면 `적용 토지면적 before→after`, `source 기존값→MANUAL`, `synced_at 기존값→NULL`, `sync_job_id 기존값→NULL`을 한 이벤트에 기록하되 각 필드는 canonical `before != after`일 때만 diff에 넣는다.
...
3. `getBrAtchJibunInfo` 권한·응답 envelope·pagination과 PNU 토지구분 변환을 확인한다.
4. 개인정보 없는 합성 fixture와 승인된 실호출 PNU를 준비한다.
5. 단독, 다가구, 다세대, 혼재, 1,001건 이상 fixture를 준비한다.
6. `bylotCnt=0 + ATTACHED_COMPLETE_ZERO`, `bylotCnt=0 + 기존 positive evidence`, cache 없음+attached row, LINKED multi-PNU fixture를 준비한다.
7. 다른 기준지번의 부속 PNU이면서 자체 title·`bylotCnt=0`·anchor 기준 attached zero인 reverse-only fixture를 준비한다.
8. PENDING, REVIEW_REQUIRED, CONFLICT, STALE_REVIEW/STALE_CANDIDATE, OPEN unresolved와 `attached anchor+cached base+sibling`, transitive component fixture를 준비한다.
9. 단독·다가구 토지 소유와 single parcel scope를 확인할 비식별 근거 allowlist·승인자를 정한다.
10. 모든 Building HUB strict URL이 HTTPS인지 fixture에서 고정한다.
11. feature flag 기본값을 OFF로 정한다.
...
3. `getBrTitleInfo`, `getBrAtchJibunInfo`, `getBrExposInfo`, V-World strict adapter와 공통 request gate를 작성하고 모든 Building HUB URL을 HTTPS로 고정한다.
4. positive·blocking·OPEN evidence와 bounded connected component를 읽는 DB resolver client, `resolveParcelScopeCompleteness` gate를 작성한다.
5. classifier, ratio parser, identity, normalizer, matcher를 작성한다.
...
8. mock provider로 pagination·오류·partial·closed·reverse-only·single/multi-PNU를 검증한다.
...
3. `land_area=적용 토지면적`, `land_lot_area=필지 전체면적` 라벨과 source badge를 적용한다.
...
- 부속-only anchor + 자체 title + `bylotCnt=0` + `ATTACHED_COMPLETE_ZERO`가 `SINGLE_SCOPE_CONFIRMATION_REQUIRED`
...
- `bylotCnt=0 + attached row`, positive cache evidence, PENDING/blocking/OPEN은 적용 0
- `bylotCnt>0/null/누락/파싱 실패`와 attached distinct count의 조합
...
- `ATTACHED_COMPLETE_ZERO`, `LDAREG_COMPLETE_ZERO`, `LADFRL_COMPLETE_ZERO`, `TITLE/EXPOS_COMPLETE_ZERO` outcome 분리
...
- `land_area`는 `적용 토지면적`, `land_lot_area`는 `필지 전체면적`으로 구분
...
| `bylotCnt`·attached count·cache PNU 모순 | `REVIEW_REQUIRED`, 관계 변경·apply 0 |
...
- [ ] `bylotCnt=0`, positive-cache 무행 중 어느 하나도 단독으로 단일 PNU 증거가 되지 않는다.
- [ ] no-cache 부속-only anchor는 자체 title·`bylotCnt=0`·`ATTACHED_COMPLETE_ZERO`여도 자동 single이 되지 않는다.
...
- [ ] attached zero와 LDAREG/LADFRL/title/expos zero가 서로 다른 outcome으로 처리된다.
...
- [ ] `적용 토지면적`과 `필지 전체면적`의 라벨·원천·fallback이 UI·audit·export에서 일관된다.
