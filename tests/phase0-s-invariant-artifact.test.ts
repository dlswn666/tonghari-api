...
test('verify CLI 2종은 내부 verifier가 green인 FIXTURE before/after pair도 거부한다', () => {
    ...
    try {
        ...
        assert.match(invariant.stderr, /requires DISPOSABLE_CLONE/);
        ...
        assert.match(memberImport.stderr, /requires DISPOSABLE_CLONE/);
    } finally {
...
test('verify CLI source gate는 disposable clone projectRefHash가 다르면 거부한다', () => {
    ...
    assert.throws(
        ...
        /matching non-null disposable clone projectRefHash/
    );
});
